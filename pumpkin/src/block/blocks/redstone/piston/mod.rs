...
use pumpkin_data::block::{Block, BlockState, PistonBehavior};
...
impl<'a> PistonHandler<'a> {
    pub fn new(world: &'a World, pos: BlockPos, dir: BlockDirection, retracted: bool) -> Self {
        ...
        if retracted {
            ...
            pos_to = pos.offset_dir(dir.to_offset(), 1);
        } else {
            motion_direction = dir.opposite();
            pos_to = pos.offset_dir(dir.to_offset(), 2);
        }
        PistonHandler {
            world,
            pos_from: pos,
    ...
    async fn try_move(&mut self, pos: BlockPos, dir: BlockDirection) -> bool {
        ...
        let mut k = 1;
        loop {
            let block_pos2 = pos.offset_dir(self.motion_direction.to_offset(), k as i32);
